...
extension HangulAutomata {
    func hangulAutomata(key: String) {
        ...
        let isDoubleConsonant = (key == "ㄸ" || key == "ㅉ" || key == "ㅃ")
                if currentHangulState == .joongsung && isDoubleConsonant {
                    // 이전 음절은 그대로 두고
                    // 새 블록 추가
                    buffer.append("")
                    // 초성으로만 저장
                    ...
                    // 스택에도 바로 추가
                    inpStack.append(InpStack(curhanst: currentHangulState!,
                                             key: keyCode,
                                             charCode: charCode,
                                             chKind: chKind))
                    // 버퍼 갱신
        ...
        case .jongsung:
            if (chKind == .consonant) && oldChKind == .consonant && jongsungPair() {
