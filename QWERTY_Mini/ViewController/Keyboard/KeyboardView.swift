...
class KeyboardView:UIView{
    ...
    var textDocumentProxy: TextDocumentProxyWrapper?
    ...
    let infoLabel:UILabel = {
            ...
            view.text = "이 곳에서 키보드를 미리 확인해 보세요."
            return view
        }()
    ...
    private func layout() {
        [infoLabel,textView,keyboardContainer].forEach{
            self.addSubview($0)
        }
    ...
    func attribute(){
       keyboardContainer.backgroundColor = .systemGray4
        ...
        // Set up the proxy wrapper for the textView
        textDocumentProxy = TextDocumentProxyWrapper(textView: textView)
    ...
    // 2) touchesEnded 에서 남은 터치가 한 개일 때, 그 키를 다시 입력으로 처리
       ...
       private func performSingleInput(on keyView: KeyView) {
           let ch = InputManager.shared.handleTap(on: keyView, currentState, proxy)
           if currentState == .kor {
...
extension Reactive where Base:KeyboardView{
    ...
    func tap_Letter(_ keyView:KeyView){
       // print(base.currentState)
        let ch = InputManager.shared.handleTap(on: keyView,base.currentState,base.proxy)
